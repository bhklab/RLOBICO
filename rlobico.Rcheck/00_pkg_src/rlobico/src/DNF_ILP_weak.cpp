...
// [[Rcpp::export]]
List DNF_ILP_weak_cpp(DataFrame X, NumericVector Y, NumericVector W, double K, double M, double lambda,
    ...
    /* Optimizing function */
    Rcpp::Rcout << "Construction optimizing function..." << std::endl;
    ...
    Rcpp::Rcout << "Constructing constaints..." << std::endl;
    ...
    for (int n = 1; n <= N; n++){
        ...
        for (int kk = 1; kk <= K; kk++){
            ...
            if (PosSet.size() > 0){
                matrix_update_vec(J, ix_1 - 1, P * K + (kk - 1) * P + PosSet);
            }
            ...
            matrix_update_scalar(I, ix_1 - 1, conr + (kk - 1) * N + n);
            matrix_update_vec(J, ix_1 - 1, (kk - 1) * P + NegSet);
            matrix_update_scalar(Q, ix_1 - 1, 1);
            conz += LNS;
        ...
        consub[conr + (k - 1) * N + n - 1] = P;
        consub[conr + N * K + (k - 1) * N + n - 1] = -1;
    }
